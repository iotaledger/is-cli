...
export const setupNode = () => {
    ...
    const foldersExist = fs.existsSync(`${process.cwd()}/${AUDIT_TRAIL_FOLDER}`) && fs.existsSync(`${process.cwd()}/${SSI_BRIDGE_FOLDER}`)
    if (currentDirectory !== API_FOLDER) {
        return console.log(chalk.bold.red('Please navigate to the api folder'))
    }
    if (!foldersExist) {
        return console.log(chalk.bold.red(`Was not able to detect ${SSI_BRIDGE_FOLDER} and ${AUDIT_TRAIL_FOLDER} folder`))
    }
    ...
    fs.writeFileSync(`${process.cwd()}/.env`, generateEnv(dbUser, dbPassword, secrets, secrets, apiKey, '3000'));
    fs.writeFileSync(`${process.cwd()}/${AUDIT_TRAIL_FOLDER}/.env`, generateEnv(dbUser, dbPassword, secrets, secrets, apiKey, '3002'));
    fs.writeFileSync(`${process.cwd()}/${SSI_BRIDGE_FOLDER}/.env`, generateEnv(dbUser, dbPassword, secrets, secrets, apiKey, '3001'));
...
const generateEnv = (dbUser: string, dbPassword: string, serverSecret: string, jwtSecret:string, apiKey: string, port: string): string => {
    return `PORT=${port}
...
DATABASE_URL=mongodb://${dbUser}:${dbPassword}@0.0.0.0:27017
